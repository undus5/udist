...
build_iso_rootfs() {
   ...
   vfs_mount
   vfs_chroot dracut-live-install.sh
   ...
   local RD="boot/initramfs-live.img"
   [[ -f "${ROOT_FS}/${RD}" ]] || errf "==> initramfs not found: root.fs/${RD}"
   ...
   echo "==> packaging '${LIVE_IMG#$WORK_DIR/}'"
...
package_iso() {
   ...
   echo "==> packaging '${ISO_FILE#$WORK_DIR/}'"
   ...
   # dependencies: clang, lld, llvm, nasm
   # ./configure --enable-bios-cd && make
